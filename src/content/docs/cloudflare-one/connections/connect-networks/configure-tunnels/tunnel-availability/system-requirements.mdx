...
Cloudflare recommends splitting [private DNS traffic](/cloudflare-one/connections/connect-networks/private-net/cloudflared/private-dns/) into its own Cloudflare Tunnel. This is because DNS queries are [more resource intensive](#estimated-throughput) for `cloudflared` compared to TCP and non-DNS UDP traffic. To optimize service availability, the tunnel should run on a dedicated host and only include routes for your internal DNS resolver IPs.
